#!/bin/sh
#PBS -N bind_HS_UKI_Postrun_scenario_name
#PBS -A YourAccount
...
# Should read in results of previous runs, write new parameters, edit needed xml files but not submit.
julia /glade/work/teopb/hs_uq/uki_v4/calibration_scenario_files/scenario_name/scenario_name_postrun_julia_script.jl

# check if calibration is complete by checking if the complete file exists
if [ -f "${case_directory}/${case_name}_complete.txt" ]; then
    echo "Calibration is complete, exiting"
    exit 0
fi
